    ...
    <div class="card-body">
      <form id="houseForm">
        ...
        <div class="form-group">
          ...
          <input type="text" id="buildingNo" name="buildingNo" class="form-control" value="${house.buildingNo}" required>
        </div>

        <div class="form-group">
          <label for="unitNo">单元号:</label>
          <input type="text" id="unitNo" name="unitNo" class="form-control" value="${house.unitNo}" required>
        </div>

        <div class="form-group">
          <label for="roomNo">房号:</label>
          <input type="text" id="roomNo" name="roomNo" class="form-control" value="${house.roomNo}" required>
        </div>

        <div class="form-group">
          <label for="area">房屋面积(平方米):</label>
          <input type="number" step="0.01" id="area" name="area" class="form-control" value="${house.area}">
        </div>

        <div class="form-group">
          <label for="houseType">房屋类型:</label>
          <input type="text" id="houseType" name="houseType" class="form-control" value="${house.houseType}">
        </div>

        <div class="form-group">
          <label for="status">房屋状态:</label>
          <select id="status" name="status" class="form-control">
            <option value="空置" ${house.status == '空置' ? 'selected' : ''}>空置</option>
            <option value="自住" ${house.status == '自住' ? 'selected' : ''}>自住</option>
            <option value="出租" ${house.status == '出租' ? 'selected' : ''}>出租</option>
          </select>
        </div>
...
<script>
  document.getElementById('houseForm').addEventListener('submit', function(e) {
    ...
    fetch('${pageContext.request.contextPath}/house/update', {
      method: 'POST',
      headers: {
